...
本研究實證以**主資料(HRDataset_v2, N = 1,224)**為主,並另以**兩份結構迥異的公開資料集(INX Future Inc, N = 1,200;Rich Huebner's HR, N = 311)**進行跨資料集通用性驗證,佐證框架之外部效度與重現性。
...
│   └── NotoSansTC-Regular.ttf      # 中文字型檔(供圖表顯示)
...
├── cleaned_hr_data.csv         # 主資料(1,224 筆,47 欄位)
...
| 資料集 | 來源 | 樣本量 | 授權 | 用途 |
|---|---|---|---|---|
| **HRDataset_v2** | Abdallah (2024), Kaggle | 1,224(經清理) | 公開 | 本研究主資料,模型訓練與雙軌偵測 |
...
- 本研究之驗證資料集均為公開教學或合成性質,非真實企業績效紀錄,外部效度推廣至真實企業情境時仍需個案驗證。
- 偏誤來源歸因(主要與員工自評行為共變)係建立於橫斷面資料的相關性分析,嚴格因果推論需仰賴縱貫資料或實驗設計。
